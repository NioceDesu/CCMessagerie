...
// 
async function sendMessage(msg, room, date, sender, type, comptable, msgType) {
    var res = await saveMsgInDB([msg, date, sender, type, comptable, room, msgType]);
    if (res.affectedRows == 1)
        socket.emit('send message', msg, room, date, sender, type, msgType);
    else
...
// 
function sendFile(fileEl, room, date, sender, senderType, comptable) {
    var executed = false;
    ...
    ipcRenderer.on('download complete', function (event, data) {
        if (!executed) {
            ...
            sendMessage(data.name, room, date, sender, senderType, comptable, 'file');
            fileEl.value = '';
            executed = true;
        }
    });
